...
This is just a skeleton of an app based on some experimentation with ChatGPT with built in data. Once we figure out wht graphs we want to include, we just have to replace all the variable names
...
# Define UI
ui <- dashboardPage(
  dashboardHeader(title = "Species Analysis"),
  dashboardSidebar(
    sidebarMenu(
      menuItem("Species Analysis", tabName = "species_analysis", icon = icon("chart-line"))
    )
  ),
  dashboardBody(
    tabItems(
...
server <- function(input, output, session) {
  observe({
    updateSelectInput(session, "species",
                      choices = setdiff(unique(iris$Species), input$habitat))
  })
  ...
  selected_data <- reactive({
    if (!is.null(input$species)) {
      ...
      iris[iris$Species == input$habitat, ]
    }
  })
  
  plot_type <- reactiveVal("sepal_vs_petal")
  
  output$selected_plot <- renderPlot({
    plot_func <- switch(plot_type(),
                        "sepal_vs_petal" = {
                          ggplot(selected_data(), aes(x = Sepal.Length, y = Petal.Length, color = Species)) +
                            geom_point() +
                            labs(x = "Sepal Length", y = "Petal Length") +
                            ggtitle("Sepal Length vs Petal Length")
                        },
                        "sepal_width_vs_petal_width" = {
                          ggplot(selected_data(), aes(x = Sepal.Width, y = Petal.Width, color = Species)) +
                            geom_point() +
                            labs(x = "Sepal Width", y = "Petal Width") +
                            ggtitle("Sepal Width vs Petal Width")
                        },
                        "sepal_length_vs_width" = {
  ...
  observeEvent(input$switch_button, {
    if (plot_type() == "sepal_vs_petal") {
      plot_type("sepal_width_vs_petal_width")
    } else if (plot_type() == "sepal_width_vs_petal_width") {
      plot_type("sepal_length_vs_width")
    } else {
      plot_type("sepal_vs_petal")
    }
  })
}
